...
import jobportal.dao.PostJobDao;
...
public class PostJob extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req,HttpServletResponse res) {
		...
		if(hs==null) {
			...
			String experience=req.getParameter("exp").toString();
			Short vacancy=Short.parseShort(req.getParameter("vacancy"));
			Integer salary=Integer.parseInt(req.getParameter("sal"));
		...
		i=new PostJobDao().postJob(slno,title,companyName,department,batches,experience,vacancy,salary,recruiterMailid,sqldate,location,description,skills);
		...
		if(i>0) {
			...
			req.getRequestDispatcher("postjobsuccess.jsp").forward(req, res);
		}else {
			req.setAttribute("msg","Not Posted please try again");
			req.setAttribute("companyName", companyName);
			req.getRequestDispatcher("postjob.jsp").forward(req, res);
		}
		}catch(Exception e) {
			e.printStackTrace();
		}
